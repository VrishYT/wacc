...
class HeapAllocator {
     ...
     def mallocPair(fst: Operand, snd: Operand, out: Register): Assembly = {
        ...
        def mallocPairElem(elem: Operand): Seq[Instruction] = {
            val instrs = (Mov(Register(0), ImmInt(elemSize)) +: HeapAllocator.malloc) ++ Seq(Mov(out, Register(0)),  Mov(Register(8), elem))

            val rest = elem match {
                case ImmChar(_) => Seq(StoreB(Register(8), Address(out, ImmInt(0))), Mov(Register(8), out), HeapAllocator.push)
                case _ => Seq(Store(Register(8), Address(out, ImmInt(0))),  Mov(Register(8), out), HeapAllocator.push)
            }
            
            return instrs ++ rest
        }

        val instrs = Seq(HeapAllocator.push) ++ mallocPairElem(fst) ++ mallocPairElem(snd) ++ Seq(Mov(Register(0), 
                                                                                    ImmInt(elemSize * 2))) ++ 
                                                                                    HeapAllocator.malloc ++ Seq(
                                                                                     Mov(out, Register(0)),
                                                                                    HeapAllocator.pop, 
...
object HeapAllocator extends HeapAllocator(){
    val malloc = Seq(
